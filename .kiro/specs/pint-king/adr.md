...
| 0107 | The offline queue is a standalone `PintUploadQueue` over three seams (store, transport, connectivity) rather than logic inside a repository that does not exist yet; one directory with two files per pint, photo written before metadata so the metadata *is* the commit point; being offline costs no retry attempt — only a server rejection does — with 3 rejections or 24h in the queue as the two give-up conditions; uploads go through `NetworkClient` on the foreground path, **not** `URLSessionConfiguration.background` | Accepted (revisit) |
...
## ADR-0107: The offline queue is a standalone component over three seams; the photo write is the commit point; being offline costs no retry attempt; and the upload is not a background URLSession
...
Task 29 is the last piece of functionality before the integration pass. The task text says "implement the offline queue **in PintRepository**", and the design (l3-ios-app.md §8, ADR-0023) specifies the behaviour: photo and metadata saved locally on capture, a pending badge in My Pints only, background upload when connectivity returns "even if the user leaves the app or locks their phone" via `URLSessionConfiguration.background`, and a give-up rule of "3 retries over 24 hours" after which the pint is marked failed and offered retry / discard.

Four things are in tension with writing that literally:

1. **There is no `PintRepository`.** Tasks 26 and 27 built `NetworkClient` and `KeychainService` and deliberately left them unwired — every screen is still backed by a mock, and the real repositories land when the backend does. Putting the queue "in PintRepository" would mean writing that repository now, which drags in the two DTO mismatches Task 26 found (`User.appleId` absent from `GET /users/me`; `PintLog` nesting a `Coordinate` where the API sends flat lat/lng) and five other endpoints that have nothing to do with queueing.
2. **What actually needs testing is the policy, not the plumbing.** "When do we try, how often, when do we stop" is decidable without a network, a disk of consequence, or a device — but only if none of those three are reached for directly.
3. **`URLSessionConfiguration.background` fights the token layer.** A background upload task is handed to the system and may run minutes or hours later, in a process the system relaunched. Its `URLRequest` — including its `Authorization` header — is frozen at the moment it is created. ADR-0104 put JWT stamping, proactive refresh and refresh-on-401-then-retry inside `NetworkClient`, all of which need to run *at send time*. A queued pint is precisely the request most likely to be sent with an expired token, and a background task cannot re-stamp itself. It also cannot be exercised by a unit test at all.
4. **Found while reading `PintController`:** `POST /pints` takes no group id — it logs into the caller's *active* group — and stamps `logged_at` server-side, accepting no client timestamp.

### Decision

**The queue is `PintUploadQueue`, a `@MainActor @Observable` class in `Core/Offline/`, standing on three injected seams: `PintQueueStore` (a real struct over the file system), `PintUploading` (transport), and `NetworkMonitoring` (connectivity).** It owns the in-memory mirror of the queue, the decision of *when* to attempt, and the decision of *when to stop trying* — nothing else. Its three public members that a repository will need are spelled exactly as `PintRepositoryProtocol` spells them (`pendingPints`, `retryPint(pintId:)`, `discardPint(pintId:)`), so the real repository forwards them verbatim rather than reimplementing them. Like Tasks 26 and 27, nothing is wired at runtime; `MockPintRepository` still backs My Pints.

**Storage is one directory with two files per pint — `<id>.jpg` and `<id>.json` — and the photo is written first.** A file per pint rather than a single index means recording an attempt count is an atomic write of one small file instead of a rewrite of the whole queue, so a crash mid-write can corrupt at most the record being touched. The write order makes the JSON the commit point: **a record that exists is always uploadable.** An interrupted save leaves an orphan photo, never a metadata row pointing at a file that is not there — the same "store the blob, then the row" ordering the API uses for S3 (ADR-0001). `load()` enforces the invariant from the other side: it drops (and deletes) metadata whose photo is missing or whose JSON no longer decodes, and sweeps photos with no surviving metadata. That makes it the queue's only garbage collector, which it has to be — a queued photo is up to 10 MB and nothing else ever walks the directory.

**Being offline costs no retry attempt; only a server rejection does.** `processQueue()` returns immediately if the monitor says there is no path, and a `.networkUnavailable` thrown by a request that *was* sent (connectivity died mid-flight) also costs nothing and stops the whole pass, because every remaining pint would fail identically. Attempts count rejections — the cases where the server saw the request and said no. Three of those marks the pint failed. This is what makes the design's "3 retries" mean something: counting offline attempts would burn all three on three launches in a tunnel, without a single request ever leaving the device.

**The 24h window is the backstop for the case attempts cannot catch, and it is measured from `queuedAt`, not `capturedAt`.** A device that is simply never online accrues no attempts, so without a deadline its queue would grow forever; a pint that has sat for more than 24 hours is marked failed without being tried. `queuedAt` is reset by a user-initiated retry (along with the attempt count), because a Retry button that reports failure without trying anything would be a lie. `capturedAt` stays fixed as the pint's displayed timestamp.

**Uploads go through `NetworkClient` on the foreground path, not a background `URLSession`.** `NetworkPintUploader` builds the multipart body and calls `client.upload(.createPint, form:)`, which gets it token stamping and refresh-on-401 (ADR-0104) — the thing a frozen background request cannot have. Draining is triggered by the offline → online edge (`NWPathMonitor`, via `NetworkMonitoring.onConnectionRestored`) and by an explicit `processQueue()` the app calls on foreground. **This is a deliberate deviation from the task text and from design §8**, and it costs the "uploads while the app is closed" half of the promise: a pint queued offline uploads the next time the app is open with a network, not the moment the phone reconnects in the user's pocket. See Consequences for what taking it back would require.

**`QueuedPint` does not use `JSONCoding`.** The shared coder is documented as the single source of truth for the *wire* format, and its `.iso8601` date strategy is second-resolution. A queued pint never leaves the device, and round-tripping a `capturedAt` through ISO-8601 rounds it — the record that comes back is not the record that was saved. The store uses its own `JSONEncoder`/`JSONDecoder` with Foundation's default `Double` date strategy, which round-trips exactly. (Caught by `savedPintLoadsBackUnchanged`, which is the test being an equality assertion rather than a field-by-field one.)

**`QueuedPint` is a separate type from `PintLog`, and `PendingPint` is derived from it.** A `PintLog` carries server-assigned facts — its id, its S3 `photoUrl`, the `loggedAt` the server stamped — that a queued pint does not have, so persisting one would mean inventing them on disk. The projection fills them in only at the moment the UI asks: the local file URL stands in for the S3 one, and `capturedAt` stands in for `loggedAt`.

### Alternatives Considered

- **Writing the real `PintRepository` now and putting the queue inside it (the task text as written):** rejected — it forces the two known DTO mismatches and five unrelated endpoints into a task about queueing, and none of the queue's policy would be any better tested for it. The seam is already named: the three protocol members exist and the queue implements them.
- **Wiring the queue into `MockPintRepository` so My Pints shows real queued pints:** rejected — the mock's "upload" is an in-memory append that cannot fail, so the connectivity and retry paths it would exercise are exactly the ones that do not exist there. It would also make the mock's seeded pending/failed fixtures fight real persisted state.
- **A genuine `URLSessionConfiguration.background` upload:** rejected for now, on three counts — the frozen `Authorization` header (§Context 3), the `UIApplicationDelegateAdaptor` + `handleEventsForBackgroundURLSession` machinery it needs to complete correctly, and that no unit test can reach it. Its own automatic retry would also overlap confusingly with ours.
- **One `queue.json` index file instead of a file per pint:** rejected — every attempt-count bump would rewrite the whole queue, so a crash at the wrong moment loses records that had nothing to do with the write.
- **Base64-ing the photo into the metadata JSON:** rejected — it inflates a 10 MB photo by a third and forces the whole record to be read to answer "is this pint still uploadable".
- **Writing metadata before the photo:** rejected — it makes the failure mode a record that can never be uploaded and can only be discovered by trying, instead of an orphan file that `load()` sweeps silently.
- **Counting an offline attempt as an attempt:** rejected — see Decision; it makes "3 retries" a count of launches rather than of rejections.
- **Measuring the 24h window from `capturedAt`:** rejected — the user is only offered Retry *after* the pint has failed, which by definition is late in its life; anchoring to capture makes the button unable to do anything for the pints it appears on.
- **Failing fast on a non-retryable 4xx (403 not a member, 404 group gone):** not done — it is correct and cheap, but it is a second policy in a component whose whole point is one policy, and the cost of not having it is at most two wasted requests. Listed as a revisit rather than snuck in.
- **Doing the disk I/O off the main actor:** not done — the queue is `@MainActor` like every other observable object in the app, and reads happen once per upload attempt. Listed as a revisit.

### Consequences

- 34 new tests (12 `PintQueueStoreTests`, 22 `PintUploadQueueTests`). Full suite **419 pass**.
- The store tests use the real file system on a per-suite temporary directory, for the same reason the keychain tests use the real keychain (ADR-0105): the I/O *is* the risk being covered. The queue tests inject a clock, so "25 hours later" costs nothing.
- "Survives app restart" is testable without restarting anything: a second `PintUploadQueue` over the same directory is exactly what the next launch builds, and three tests assert through one (`aQueuedPintSurvivesARestart`, `attemptCountsSurviveARestart`, and the disk half of `aSuccessfulUploadLeavesTheQueue`).
- `PendingPint.Status` gained a `String` raw value and `Codable`, because it is now persisted.
- `PintResponse` (flat lat/lng → `Coordinate`) lands in `PintUploading.swift`, closing the first half of Task 26's "found in passing" note at the first call site that needed it rather than in the domain model.
- **Two API gaps that only the queue exposes, both unresolved:**
  - **`POST /pints` derives the group from the caller's *active* group and takes no group id.** A pint captured in group A while offline, uploaded after the user switches to group B, lands in **B**. The queue records `groupId` so the local row is filed correctly, but it has no way to make the server agree. The fix is an API change (accept `group_id`); the client-side workaround — PATCH the active group, upload, PATCH it back — mutates user-visible state as a side effect of a background upload and is not worth it.
  - **`logged_at` is stamped server-side.** A pint captured Friday night and uploaded Saturday morning is a Saturday pint as far as the leaderboard's weekly period and the 24h delete window are concerned. The queue keeps `capturedAt` for local display, so the row's timestamp will visibly change when the upload confirms.
- **Revisit when:** (a) the real `PintRepository` lands — that is when `createPint` grows its "try, and on `.networkUnavailable` enqueue" branch, when `pendingPints`/`retryPint`/`discardPint` get forwarded, and when something calls `processQueue()` on `scenePhase == .active`; (b) the two API gaps above are worth an API change; (c) the background-upload promise is taken back up, which needs the frozen-header problem solved first (a fresh JWT minted just before the task is created, plus accepting that a 401 fails the attempt rather than refreshing); (d) fail-fast on non-retryable 4xx and off-main disk reads, both deferred above; (e) `POST /pints` gains an idempotency key — today a retry after a response that was persisted but lost in transit creates a duplicate pint.

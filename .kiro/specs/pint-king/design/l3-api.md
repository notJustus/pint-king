# 04 — API Server (C4 Level 3)

## Purpose

This document describes the internal structure of the API container: its endpoints, response conventions, async cleanup strategy, S3 access patterns, server-side error handling, correctness properties, and API testing approach.

For the API's place in the broader system, see [`02-containers.md`](./02-containers.md). For schema details, see [`05-database.md`](./05-database.md). For decision rationale, see `../adr.md`.
...
| POST | `/auth/logout` | Invalidate all refresh tokens for the user |
...
- **Success codes**: 200 (GET/PATCH), 201 (POST create), 204 (DELETE)
- **Client error codes**: 400 (validation), 401 (auth), 403 (forbidden), 404 (not found), 409 (conflict), 422 (unprocessable)
- **Server error codes**: 500 (internal)
- **Pagination**: Offset-based with `page`, `size`, and `total` in the response envelope
- **Timestamps**: ISO 8601 UTC (`2024-01-15T14:30:00Z`)
- **IDs**: UUID v4 as canonical hyphenated strings

---

## 3. S3 Access Patterns

The API is the only writer and reader against S3.
...
- All photos are stored as JPEG. HEIC and PNG are converted on upload.
- Read access is via pre-signed URLs (15-minute expiry) generated by the API.
- Write access is via direct multipart upload from the client to the API; the API performs the `PutObject` to S3.

### Why API-Mediated Uploads (not Direct-to-S3)

Pre-signed PUT URLs would allow the client to upload directly to S3 without API bandwidth. We do not use them in MVP because the API needs to server-side validate file type and size (per requirement 7.6) before any S3 cost is incurred. This may be revisited post-MVP via a presign-then-confirm flow. See `adr.md`.

---

## 4. Async Cleanup Strategy

Some operations cross the DB ↔ S3 boundary and cannot be transactional. The API handles these via ordering choices and async cleanup.

### Pint Creation: S3-First

1. API receives multipart request, validates JWT, validates file (type, size).
2. API uploads photo to S3 (`PutObject`).
3. On S3 success: API inserts `pint_logs` row referencing the photo key.
4. On S3 failure: API returns 500; no DB row exists.
5. On DB-insert failure after S3 success: the orphaned S3 object is enqueued for async cleanup.

The orphaned-object case is rare but possible (e.g. DB transient failure between S3 success and INSERT). It is handled, not prevented.

### Pint Deletion: DB-First

1. API receives DELETE request, validates JWT, checks 24h deletion window.
2. API deletes the `pint_logs` row inside a transaction.
3. On DB success: an async S3 `DeleteObject` is dispatched.
4. On S3 failure: the orphan-cleanup job picks it up later.

DB-first because: blocking the user on S3 latency would degrade perceived performance; partial failures (DB deleted, S3 still present) are recoverable; the inverse (S3 deleted, DB still references it) would show users a broken image.

### Account Deletion

1. Single DB transaction: delete user, all their `pint_logs`, all their `refresh_tokens`, all their `group_members` records.
2. Group admin reassignment (promote longest-standing member) or group deletion (if no other members) happens inside the same transaction.
3. On DB commit: an async batch of S3 `DeleteObject` operations is dispatched for the user's avatar and all their pint photos.
4. Dead-letter queue captures S3 failures for manual review.

The DB cascade is atomic. The S3 cleanup is best-effort with a recovery path. This satisfies requirement 8.6 ("no partial deletion state") at the DB level while accepting that S3 cleanup is eventually consistent.

### Orphan Cleanup Job

A scheduled job (cron or SQS consumer) scans S3 for objects with no corresponding `pint_logs.photo_url` or `users.avatar_url` reference and deletes them. This is the safety net for all three of the above flows.
...
|----------|----------|--------------|
| Invalid or expired JWT | 401 | None |
| Reused refresh token | 401 | Invalidate ALL refresh tokens for the user (security) |
| Unauthorized group access | 403 | None |
| Blocked user join attempt | 403 | None |
| Invalid request body | 400 + field errors | None |
| Invalid file type or size | 422 | None |
| Duplicate group membership | 409 | None |
| S3 upload failure during pint creation | 500 | No DB row created |
| DB insert failure after S3 upload | 500 | Orphaned S3 object enqueued for cleanup |
| Account deletion partial failure | Rollback | Transaction ensures DB atomicity; S3 cleanup is async with DLQ |

---

## 6. Correctness Properties

*A property is a characteristic or behaviour that should hold across all valid executions of a system — a formal statement about what the system must do. Properties serve as the bridge between human-readable requirements and machine-verifiable correctness guarantees.*

Each property is tagged with the requirement(s) it validates. Properties marked **PBT** are implemented as property-based tests (Kotest's `kotest-property` for the API). Others are covered by example-based tests.
...
### Property 6: File upload validation **(PBT)**
...
*Note: implemented client-side; tested in `03-ios-app.md` testing strategy.*
...
### Property 10: Invite code format invariant **(PBT)**
...
### Property 11: Group join outcome correctness **(PBT)**
...
### Property 16: Active group invariant **(PBT)**
...
### Property 17: Pint creation integrity **(PBT)**
...
### Property 18: Pint metadata validation **(PBT)**
...
### Property 19: Pint deletion time window **(PBT)**
...
### Property 20: Leaderboard ranking with dense ties **(PBT)**
...
### Property 21: Time-period filtering correctness **(PBT)**
...
### Property 22: Rank delta computation **(PBT)**
...
### Property 24: Bounding-box spatial query correctness **(PBT)**
...
### Property 26: Authorization enforcement **(PBT)**
...
### Property 28: Account deletion cascade **(PBT)**
...
Example-based tests for specific cases and edge cases:
...
For each property marked **(PBT)** in §6 above. Configuration:
...
End-to-end flows against real (containerised) infrastructure:

- **Auth flow**: Sign in with Apple mock → token issuance → refresh → expiry.
- **Pint creation flow**: Photo upload to S3 (LocalStack) → DB insert → verify both exist.
...
- Staging deploy on merge to `main`; production deploy is a manual gate.